...
void main() {
  ...
  group('Kartencache', () {
    test('erfolgreiches Laden speichert die Karten lokal', () async {
      ...
      final cached = jsonDecode(prefs.getString('subsumo.due_cards_cache')!) as List;
      expect(cached, hasLength(1));
      expect(cached.first['slug'], 'zr-at-angebot');
    });

    test('Netzfehler faellt auf den zuletzt geladenen Stapel zurueck', () async {
      var callCount = 0;
      final app = buildState((request) async {
        callCount++;
      ...
      final ok = await app.loadDueCards(); // zweiter Aufruf: simuliert offline

      expect(ok, isFalse);
      expect(app.dueCards, hasLength(1), reason: 'Alte Karten duerfen nicht verschwinden');
      expect(app.dueCardsFromCache, isTrue);
      expect(app.error, contains('Offline'));
    });

    test('Cache wird beim Start wiederhergestellt, bevor das Netz antwortet', () async {
      SharedPreferences.setMockInitialValues({
    ...
    test('ohne Cache und ohne Netz bleibt die Fehlermeldung eindeutig', () async {
      final app = buildState((request) async {
        throw Exception('Netzwerk nicht erreichbar');
      });
  ...
  group('Review-Outbox', () {
    test('Bewertung wird sofort persistiert, bevor der Server antwortet', () async {
      ...
      final app = buildState((request) async {
        if (request.url.path == '/v1/reviews/batch') {
          ...
          completer.add(null);
          return _json({'applied': 1, 'duplicates': 0, 'unknown_cards': [], 'results': []});
        }
        return _json({}, status: 404);
      });
    ...
    test('erfolgreich gesendete Bewertung verschwindet aus der Outbox', () async {
      final app = buildState((request) async {
        if (request.url.path == '/v1/reviews/batch') {
          return _json({'applied': 1, 'duplicates': 0, 'unknown_cards': [], 'results': []});
        }
    ...
    test('fehlgeschlagener Versand laesst die Bewertung in der Outbox', () async {
      final app = buildState((request) async {
        throw Exception('Netzwerk nicht erreichbar');
      });
      app.dueCards = [Map<String, dynamic>.from(_dueCard)];
    ...
    test('Outbox wird beim Start wiederhergestellt und automatisch gesendet '
        'sobald online', () async {
      ...
      expect(flushed, isTrue, reason: 'Die wiederhergestellte Bewertung wurde nie gesendet');
      expect(app.outbox, isEmpty, reason: 'Nach erfolgreichem Versand nicht mehr in der Outbox');
    });

    test('Outbox wird beim Start wiederhergestellt, auch wenn der '
      ...
      expect(app.outbox, hasLength(1), reason: 'muss aus SharedPreferences wiederhergestellt sein');
    ...
    test('Outbox uebersteht signOut und signIn', () async {
      ...
      expect(app.outbox, hasLength(1), reason: 'Abmelden darf keine Bewertungen loeschen');
    });

    test('beschaedigter Outbox-Eintrag laesst die App trotzdem starten', () async {
